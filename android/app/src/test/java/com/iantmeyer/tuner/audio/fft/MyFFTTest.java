...
public class MyFFTTest {
    ...
    public void reverseBits() throws Exception {
        Method method = MyFFT.class.getDeclaredMethod("reverseBits", Integer.TYPE, Integer.TYPE);
    ...
    public void fft() throws Exception {
        ...
        input[0] = new double[]{0, 1, 2, 3, 4, 5, 6, 7};
        input[1] = new double[]{0, 0, 0, 0, 0, 0, 0, 0};
        double[] answers_r = new double[]{28, -4, -4, -4, -4, -4, -4, -4};
        double[] answers_i = new double[]{0, 9.6569, 4.0000, 1.6569, 0, -1.6569, -4.0000, -9.6569};

        MyFFT fft = new MyFFT(input[0].length);

        long start = System.nanoTime();
        fft.fft(input);
        System.out.print("FFT time = " + (System.nanoTime() - start) + " ns");

        for (int idx = 0; idx < input[0].length; idx += 2) {
            Assert.assertEquals(answers_r[idx], input[0][idx], delta);
            Assert.assertEquals(answers_i[idx], input[1][idx], delta);
        }
